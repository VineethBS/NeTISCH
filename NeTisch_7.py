...
def make_dotfile(Graph, filename='dot'):
    graph = Graph.copy()
    for n in graph:                                                                                           
        graph.node[n]['pos'] = '"%f,%f!"'%(graph.node[n]['node_pos_x'], graph.node[n]['node_pos_y'])
...
def get_pos(graph):
    px = nx.get_node_attributes(graph, 'node_pos_x')
    py = nx.get_node_attributes(graph, 'node_pos_y')
    #print p
    pos = {}
    for n in px: 
        pos[n]=(px[n],py[n])
    ...
    return(pos)
    print('Leaving get_pos')

def edge_calc(graph, limit, param = 'distance'):
    print('Entering edge_calc')
    valid_edges = []
    if param == 'distance':
        pos = get_pos(graph)
        print('\npos: \t',pos)
...
# =============================================================================
    ...
    return (valid_edges)
    print('Leaving edge_calc')

def draw_graph(graph, filename="graph.png"):
    print(' Entering draw_graph ')
    my_dpi = 100
    ...
    nx.draw(graph, node_size = min(300, 300*150/len(type_map)), font_size= min(12, int(12*150/len(type_map))),
            pos= get_pos(graph), with_labels = True, node_color = colour_map)  
        ...
        for key1,val1 in node_data.items():
            if (val1=="Nodes"):
                for val2 in val1["Nodes"]:
                    G.add_node(val2["node_id"])
                    for key3 in val2:
                        print('\nval2[key3] : \t',val2[key3])
                        G.nodes[val2["node_id"]][key3] = val2[key3]
