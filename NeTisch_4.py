...
class MasterFrame(wx.Frame):
    def __init__(self,*args,**kwargs):
        ...
        path1 = wx.TextCtrl(self.properties_panel,-1,style=wx.ALIGN_LEFT|wx.TE_READONLY)
        sizer.Add(path1,pos=(2,1),span=(1,7),flag=wx.EXPAND|wx.LEFT, border=5)
...
##      self.nav_canvas.Bind(FloatCanvas.EVT_LEFT_DOWN, self.OnLeftDown)
    ...
    def OnGenCQ(self,event):
        self.SetStatusText("Gen C.Q - "+ event.GetEventObject().GetStringSelection())

    def OnOptimize(self,event):
        self.SetStatusText("Optimize - "+ event.GetEventObject().GetStringSelection())

    def OnSchedule(self,event):
        self.SetStatusText("Schedule - "+ event.GetEventObject().GetStringSelection())
    ...
    def OnClickBrowse(self, event):
        ...
        print("File: " + openFileDialog.GetPath())
        self.SetStatusText(openFileDialog.GetPath())
        openFileDialog.Destroy()
    
if __name__ == "__main__":
    app = wx.App()
    frame = MasterFrame(None,title = "Netisch-GUI-SampleCode-4")
    frame.Show()
    app.MainLoop()
