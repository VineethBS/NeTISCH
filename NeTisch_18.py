        ...
        for i in range (1,len(N)):
            for j in range (1,len(N)):
                if i != j:
                    ...
                    if dist < L[len(L)-1]:
                        edges.append(FloatCanvas.Line([(X[i],-Y[i]),(X[j],-Y[j])], LineColor = 'Black', LineStyle='Solid',LineWidth=1))
                        self.Canvas.AddObject(edges[-1])
                        self.Canvas.Draw()
                        E.append((i,j))
        self.draw_nx_graph(E)

    def draw_nx_graph(self,E):
##        G = nx.Graph()
        self.G.clear()
...
# Dialog Box to input the threshold distance        
    def OnGenCQ(self,event):
        ...
        if dlg_limit.ShowModal() == wx.ID_OK:
            L.append(int(dlg_limit.GetValue()))
            self.navcan.draw_lines(L)
        dlg_limit.Destroy()
    ...
    def OnClickBrowse(self, event):
        ...
        openFileDialog = wx.FileDialog(self,"Open","","","*.json",wx.FD_OPEN | wx.FD_FILE_MUST_EXIST)
